...
NSString * const ASLEncryptionParametersErrorDomain = @"ASLEncryptionParametersErrorDomain";
...
+ (instancetype)encryptionParametersWithScheme:(uint8_t)scheme
	...
	try {
		...
		if (error != nil) {
			...
			*error = [[NSError alloc] initWithDomain:ASLEncryptionParametersErrorDomain
												code:ASLEncryptionParametersErrorCodeInvalidArgument
...
- (BOOL)setPolynomialModulusDegree:(size_t)polynomialModulusDegree
	...
	try {
		...
		if (error != nil) {
			NSString * const whichParameter = [NSString stringWithUTF8String:e.what()];
			*error = [[NSError alloc] initWithDomain:ASLEncryptionParametersErrorDomain
												code:ASLEncryptionParametersErrorCodeLogicError
											userInfo:@{NSDebugDescriptionErrorKey : whichParameter}];
		}
		return NO;
	}
