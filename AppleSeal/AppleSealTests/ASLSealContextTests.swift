...
class ASLSealContextTests: XCTestCase {
	...
	func testCreateWithEncryptionParams() throws {
		...
		XCTAssertNoThrow(try ASLSealContext(encrytionParameters: encryptionParameters, expandModChain: true, securityLevel: .TC128, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
	}
	
	func testCreateWithExpandModChain() {
		let encryptionParameters = ASLEncryptionParameters(schemeType: .CKKS)
		XCTAssertNoThrow(try ASLSealContext(encrytionParameters: encryptionParameters, expandModChain: true, securityLevel: .TC128, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
		XCTAssertNoThrow(try ASLSealContext(encrytionParameters: encryptionParameters, expandModChain: false, securityLevel: .TC128, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
	}
	
	func  testCreateWithSecurityLevel() {
        XCTAssertNoThrow(try ASLSealContext(encrytionParameters: createEncryptionParameters(), expandModChain: true, securityLevel: .None, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
        XCTAssertNoThrow(try ASLSealContext(encrytionParameters: createEncryptionParameters(), expandModChain: true, securityLevel: .TC128, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
        XCTAssertNoThrow(try ASLSealContext(encrytionParameters: createEncryptionParameters(), expandModChain: true, securityLevel: .TC192, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
        XCTAssertNoThrow(try ASLSealContext(encrytionParameters: createEncryptionParameters(), expandModChain: true, securityLevel: .TC256, memoryPoolHandle: ASLMemoryPoolHandle(clearOnDestruction: true)))
	}
